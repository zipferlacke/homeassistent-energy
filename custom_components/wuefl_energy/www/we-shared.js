...
/**
 * Alle Gesamtzähler der Zuordnung – die eine Liste, an der sich Import,
 * Export und die Grenzen des Kalenders ausrichten.
 *
 * Wer hier fehlt, wird weder importiert noch beim Suchen nach dem ersten
 * Wert berücksichtigt; darum steht die Liste an einer Stelle.
 */
export function energyTargets(cfg) {
  const out = [];
  const add = (kind, label, value) => {
    for (const entity of asList(value).map(entityOfValue).filter(Boolean)) {
